...
- **Remote id** (`JSONRPCSubscriptionId`, `number`/`string`) — a subscription id the node returns in the response to a `*_subscribe` call. Stored on the `JSONRPCSubscribing` as `remoteId`; update notifications are matched by it, and the wire unsubscribe **echoes it in its original type** — jsonrpsee servers key subscriptions by an untagged number-or-string, so a stringified number would never match a numeric key.
- `subscribe(..., onSubscribed:)` delivers the remote id to the caller when the subscribe response arrives — and again with the NEW id after every reconnect resubscribe (the rebind signal).
- `cancelForIdentifiers(_:sendUnsubscribe: false)` frees a subscription without the wire unsubscribe — for streams the server already terminated.
...
unsubscribe on cancel, numeric remote-id round-trip (typed capture, update
routing, numeric wire unsubscribe params), `onSubscribed` reveal + refire after
reconnect, silent cancel (`sendUnsubscribe: false`), idempotent subscription
replay across reconnect, non-idempotent subscription cancellation on reconnect,
give-up failing pending requests, node switching via `JSONRPCNodeSwitching`,
graceful disconnect, inbound ping→pong.
